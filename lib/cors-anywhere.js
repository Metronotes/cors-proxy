...
function proxyRequest(req, res, proxy) {
  ...
  let cachePeriod = req.corsAnywhereRequestState.cachePeriod;
        ...
        proxyReq.on = function(eventName, listener) {
          ...
          return proxyReqOn.call(this, 'response', function(proxyRes) {
            if (onProxyResponse(proxy, proxyReq, proxyRes, req, res)) {
              try {
                if(cacheProxy != undefined) {
                  cacheProxy.set(location, proxyRes.body, cachePeriod);
                }
                listener(proxyRes);
              } catch (err) {
...
function parseURL(req_url) {
  var match = req_url.match(/^(?:(https?:)?\/\/)?(([^\/?]+?)(?::(\d{0,5})(?=[\/?]|$))?)(\/\d{0,2}m)?([\/?][\S\s]*|$)/i);
  //                              ^^^^^^^          ^^^^^^^^      ^^^^^^^               ^^^^^^^^^^^^^
  //                            1:protocol       3:hostname     4:port                 5: cache interval     6:path + query string
  //                                             ^^^^^^^^^^^^^^^^^^^^^^^
  //                                             2:host
  if (!match) {
    return null;
...
function parseLocation(location) {
  ...
  var match = location.match(/^(\/\d{0,2}m)?([\/?][\S\s]*|$)/i);

  res.path = match[2].split(1);
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    let cache = proxyCache.get(req.url)
    ...
    locationInfo = parseLocation(location);

    req.corsAnywhereRequestState.location = locationInfo.path;
    req.corsAnywhereRequestState.cachePeriod = locationInfo.cachePeriod;
